<template>
  <div class="page-container">
    ...
    <div style="margin-top: 50vh">
      ...
      <!-- 数据加载完成但为空 -->
      <div v-else-if="diaryList.length === 0" class="empty-state">
...
const navigateToDetail = (item) => {
  if (item.id) {
    router.push({
      name: 'FrontDetail',
      params: { type: 'diary', id: item.id }
    })
  } else {
    ElNotification.warning({
      title: '提示',
      message: '该日记缺少必要的标识信息，无法查看详情'
    });
  }
};
...
const loadData = async () => {
  ...
  try {
    ...
    if (res.code === '200') {
      ...
      errorMessage.value = res.msg || '获取日记列表失败';
